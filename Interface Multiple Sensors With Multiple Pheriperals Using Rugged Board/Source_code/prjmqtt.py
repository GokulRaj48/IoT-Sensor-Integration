...
ACCESS_TOKEN = "8G9fwPHLsj1yjNUX0LaY"
...
def ldr_sensor_task():
    while running:
        ...
        if ldr_value is not None:
            ...
            status = "�🔸 Bright" if ldr_value < LIGHT_THRESHOLD else "�🔸 Dark" if ldr_value > DARK_THRESHOLD else "�🔸 Transitional"

            # Print Data
            print("\n�🔸 LDR Value: {}, Voltage: {:.2f}V -> {}".format(ldr_value, voltage, status))

            # Publish to ThingsBoard
            payload = {"ldr_value": ldr_value, "voltage": voltage, "status": status}
            client.publish("v1/devices/me/telemetry", json.dumps(payload))
        time.sleep(1)
...
def measure_distance():
    ...
    while echoPin.read() == 0:
        ...
        if start_time > timeout:
            print("�🔺 Echo timeout (no response)")
            return None
...
# === Running All Sensors Simultaneously ===
